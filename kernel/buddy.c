...
bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].alloc, k == MAXSIZE ? 1 : NBLK(k) / 2);
    if(k > 0) {
      printf("  split:");
...
bd_initfree_pair(int k, int bi, void *bd_left, void *bd_right) {
  ...
  if(bit_isset(bd_sizes[k].alloc, bi / 2)) { // B1_free XOR B2_free = 1
    // one of the pair is free
    free = BLK_SIZE(k);
    if (bi_addr >= bd_left && bi_addr + BLK_SIZE(k) <= bd_right)
